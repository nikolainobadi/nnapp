...
//        let mockPicker = MockSwiftPicker(permissionResult: .init(type: .ordered([true]))) // User says yes to using existing folder
...
//        let mockPicker = MockSwiftPicker(permissionResult: .init(type: .ordered([false]))) // User says no to using existing folder
...
//        let mockPicker = MockSwiftPicker(inputResult: .init(type: .ordered(["CreatedFromSelection"])), permissionResult: .init(type: .ordered([true])))
//        let (sut, context) = try makeSUT(picker: mockPicker)
...
//        let picker = MockSwiftPicker(permissionResult: .init(type: .ordered([true])))
...
//        let mockPicker = MockSwiftPicker(inputResult: .init(type: .ordered(["newcut"])))
//        let (sut, context) = try makeSUT(picker: mockPicker)
//        let group = makeGroup()
...
//        let mockPicker = MockSwiftPicker(permissionResult: .init(type: .ordered([false])))
//        let (sut, context) = try makeSUT(picker: mockPicker)
//        let group = makeGroup(shortcut: "main")
...
//    func makeSUT(picker: MockSwiftPicker? = nil, permissionResponses: [Bool] = []) throws -> (sut: GroupHandler, context: CodeLaunchContext) {
...
//
//        let mockPicker = picker ?? MockSwiftPicker(permissionResult: .init(type: .ordered(permissionResponses)))
